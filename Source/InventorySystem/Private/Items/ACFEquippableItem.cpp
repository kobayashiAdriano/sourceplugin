...
void AACFEquippableItem::Internal_OnEquipped(ACharacter* charOwner)
{
    if (charOwner) {
        ...
        ModifierHandle = AddGASModifierToOwner(GameplayModifier);
...
void AACFEquippableItem::RemoveModifierToOwner(const FAttributesSetModifier& inModifier)
{
    if (ItemOwner) {
        ...
        if (ModifierHandle.IsValid()) {
            RemoveGASModifierToOwner(ModifierHandle);
        }
    }
}
...
bool AACFEquippableItem::CanBeEquipped_Implementation(class UACFEquipmentComponent* equipComp)
{
    return true;
}
