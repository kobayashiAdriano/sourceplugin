...
void UACFActionsManagerComponent::FreeAction()
{
    ...
    if (StoredAction != FGameplayTag()) {
        TriggerAction(StoredAction, EActionPriority::ELow, false);
        StoreAction(FGameplayTag());
    } else {
        ExitAction();
    }
...
void UACFActionsManagerComponent::PrepareWarp()
{
    ...
    if (motionComp && MontageInfo.ReproductionType == EMontageReproductionType::EMotionWarped && MontageInfo.WarpInfo.WarpConfig.bAutoWarp) {
...
void UACFActionsManagerComponent::StartCooldown(const FGameplayTag& action, UACFBaseAction* actionRef)
{
    ...
    if (world) {
        world->GetTimerManager().SetTimer(
            actionRef->CooldownTimerReference, TimerDel, actionRef->GetActionConfig().CoolDownTime,
            false);
        onCooldownActions.Add(action);
    }
    UE_LOG(LogTemp, Warning, TEXT("Starting cooldown for action %s and timer %f"), *action.ToString(), actionRef->GetActionConfig().CoolDownTime);
}
