    ...
    if (aiController->IsPartOfGroup()) {
        ...
        if (lead) {
            ...
            if (aiController->ShouldTeleportNearLead() && distanceFromLeader >= aiController->GetTeleportLeadDistanceTrigger()) {
                ...
                aiController->GetDefaultAIState() == UACFFunctionLibrary::GetAIStateTag(EAIState::EFollowLeader)) {
                aiController->SetCurrentAIState(UACFFunctionLibrary::GetAIStateTag(EAIState::EFollowLeader));
                return;
            }
        } else {
        ...
        if (aiController->ShouldReturnHome() && aiController->GetAIState() != UACFFunctionLibrary::GetAIStateTag(EAIState::EReturnHome)) {
            ...
            if (distanceFromHome > aiController->GetMaxDistanceFromHome()) {
                aiController->SetTarget(nullptr);
                aiController->SetCurrentAIState(UACFFunctionLibrary::GetAIStateTag(EAIState::EReturnHome));
                return;
            }
        }
    }
