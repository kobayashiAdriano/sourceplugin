...
{
    return GetAIState() == UACFFunctionLibrary::GetAIStateTag(EAIState::EBattle);
}
...
void AACFAIController::SetCurrentAIState(FGameplayTag aiState)
{
    ...
    if (GetAIState() == UACFFunctionLibrary::GetAIStateTag(EAIState::EBattle) && CombatBehaviorComponent && PerceptionComponent && aiState != UACFFunctionLibrary::GetAIStateTag(EAIState::EBattle)) {

        CombatBehaviorComponent->UninitBehavior();
    }

    if (aiState == UACFFunctionLibrary::GetAIStateTag(EAIState::EFollowLeader)) {
        SetFocus(GetLeadActorBK(), EAIFocusPriority::Gameplay);
    }

    if (aiState == UACFFunctionLibrary::GetAIStateTag(EAIState::EReturnHome)) {
        SetTargetLocationBK(homeLocation);
    }

    if (aiState == UACFFunctionLibrary::GetAIStateTag(EAIState::EBattle) && CombatBehaviorComponent) {
    ...
    AACFGameState* gameState = GetWorld()->GetGameState<AACFGameState>();
    if (gameState) {
        if (aiState == UACFFunctionLibrary::GetAIStateTag(EAIState::EBattle)) {
...
void AACFAIController::HandleCharacterHit(const FACFDamageEvent& damageReceived)
{
    ...
    if (bShouldReactOnHit) {
        ...
        if (GetAIState() != UACFFunctionLibrary::GetAIStateTag(EAIState::EBattle)) {
...
{
    SetCurrentAIState(UACFFunctionLibrary::GetAIStateTag(EAIState::EWait));
    if (CharacterOwned) {
