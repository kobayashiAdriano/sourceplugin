...
void UCCMCameraFaderComponent::HandlePawnChanges(APawn* newPawn)
{
    ...
    if (!IsValid(ActiveController) || !IsValid(ActiveSpringArm) || !IsValid(ActiveCamera)) {
        UE_LOG(LogTemp, Error, TEXT("The Fader Component needs to be attached to your player controller! - UCCMCameraFaderComponent::HandlePawnChanges"));
    }
}
